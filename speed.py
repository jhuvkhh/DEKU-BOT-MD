...
def shell():
    ...
    if not args.mini:
        printer('♡♻️╎ *بدء الاختبار...*', quiet)
        printer('♡🖥️╎ *البحث عن السيرفر ...*', quiet)
        try:
            speedtest.get_servers(servers=args.server, exclude=args.exclude)
        except NoMatchedServers:
            raise SpeedtestCLIError(
                '♡❌╎ *لا يوجد سيرفر حالياً:* %s' %
                ', '.join('%s' % s for s in args.server)
            )
        except (ServersRetrievalError,) + HTTP_ERRORS:
            printer('♡💤╎ *لم يتم الحصول علي قائمة السيرفرات*', error=True)
            raise SpeedtestCLIError(get_exception())
        ...
        if args.server and len(args.server) == 1:
            printer('♡📦╎ *الحصول علي المعلومات من السيرفر...*', quiet)
        else:
            printer('♡🚀╎ *تم الحصول علي افضل سيرفر...*', quiet)
    ...
    printer('\n♡🎠╎ *أي اس بي:* %(isp)s' % speedtest.config['client'],
            quiet)
    printer('♡🦋╎ *السيرفر:* %(sponsor)s\n♡📊╎ *موقع:* %(name)s [%(d)0.2f km] '
            '\n♡⚡╎ *السرعة:* %(latency)s ms' % results.server, quiet)
    ...
    if args.share and not machine_format:
        printer('\n♡📂╎ *مشاركة النتائج:* %s' % results.share())
